...
class StoryPublish extends Component {
 ...
 constructor(props) {
    ...
    publishingId:this.props.match.params.id,
     ...
     var fetch = require('graphql-fetch')(MainApi)
          ...
          fetch(query, queryVars, opts).then(function (results) {
           ...
           if ( results.data.Publishing == null){
              ...
              uId:results.data.Publishing.user.id,
...
renderButton(){
 ...
 if(this.state.uId == localStorage.getItem('uid')){
   ...
   return(
    ...
    <div>
       ...
       {this.state.title && this.state.body && this.state.sumber &&
            <div>    
...
{this.state.title && this.state.body && this.state.sumber &&
                  ...
                  <div className="new-content">
                    ...
                    <div className="post-author-info-top">
                     <div className="main-title" style={{marginBottom:'70px'}}>
                         ...
                         <Image style={{float:'left', marginRight:'15px'}} cloudName={Cloudinary_Name} publicId={this.state.avatarId}  crop="scale"  width="50" height="50" alt="cover"/> <h4><strong>{this.state.name}</strong> New Storie </h4>
                     </div>
  ...
  handlePost = async () => {
    ...
    const { title, slug, body, imageId, imageUrl, topicsIds, typeId, headline, reading,  isPublished, publishingId, sumber } = this.state
  
    await this.props.createPostMutation({variables: { title, slug, body,  userId, imageId, imageUrl, topicsIds, headline, reading , isPublished, typeId, publishingId, sumber }})
  ...
  handleSave = async () => {
    ...
    const userId = localStorage.getItem('uid');
    const { title, slug, body, imageId, imageUrl, topicsIds, typeId, headline, reading, publishingId, sumber} = this.state
  
    await this.props.createSaveMutation({variables: { title, slug, body,  userId, imageId, imageUrl, topicsIds, headline, reading , typeId, publishingId, sumber }})
...
const CREATE_POST_MUTATIONX = gql`
...
const CREATE_SAVE_MUTATIONX = gql`
...
export default compose(
  graphql(FeedQuery),
  graphql(CREATE_POST_MUTATIONX, { name: 'createPostMutation' }),
   graphql(CREATE_SAVE_MUTATIONX, { name: 'createSaveMutation' }),
  graphql(LOGGED_IN_USER_QUERY, { 
    name: 'loggedInUserQuery',
    options: { fetchPolicy: 'network-only' }
  })
)(withRouter(StoryPublish))